...
def test_practice_with_item_appends_to_history(student, test_item):
    ...
    initial_history_length = len(student.history.get_events())

    # Practice with the specific item
    student.practice(skill=test_item.skill, item=test_item)

    # Check that history length increased
    assert len(student.history.get_events()) == initial_history_length + 1

    # Get the last event
    last_event = student.history.get_events()[-1]
...
def test_practice_with_item_records_practice_increment(student, test_item):
    """Test that practice with an item records the practice increment."""
    # Practice with the specific item
    student.practice(skill=test_item.skill, item=test_item)
    ...
    # Verify practice_increment_logit is recorded
    assert hasattr(last_event, "practice_increment_logit")
    assert (
        last_event.practice_increment_logit == test_item.skill.practice_increment_logit
    )
...
def test_multiple_practice_sessions_with_items(student, test_item):
    ...
    # Check that we have 3 new events
    assert len(student.history.get_events()) == 3

    # Check that all events have the item
...
def test_practice_with_different_items(student, simple_skill_space):
    ...
    assert len(student.history.get_events()) == 2

    # Check that the items are different
    events = student.history.get_events()
...
def test_practice_with_pool_by_name(student, simple_skill_space):
    ...
    practice_events = [
        ev
        for ev in student.history.get_events()
        if isinstance(ev, ItemResponseEvent) and ev.feedback_given
    ]
